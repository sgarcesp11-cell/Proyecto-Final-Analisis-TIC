...
* **Base de Datos:** `SQLite` (Modelo de Estrella con tablas de hechos y dimensiones).
* **Procesamiento de Datos:** `Pandas` para manipulación, agrupación y cálculo de KPIs estadísticos.
* **Visualización:** `Plotly` para gráficos interactivos (Heatmaps, Boxplots, Series de Tiempo).
* ]**Despliegue Web:** `Streamlit` para la creación del dashboard interactivo y su alojamiento en la nube.
...
* `app.py`: Script principal de Python que ejecuta la aplicación de Streamlit y renderiza las visualizaciones.
